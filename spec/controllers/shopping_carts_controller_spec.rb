...
RSpec.describe ShoppingCartsController, type: :controller do
  ...
  let!(:product_discount_rule1) do
    FactoryBot.create(
      :product_discount_rule, { multiples: 3, offer_price: 50, product_id: product1.id}
    )
  end
  context 'checkout' do
    it 'gives detailed bill on checkout' do
      expected_response =
        {
          order_items_summary: [
            {product_code: 'A' ,quantity: 3, unit_price: 20.0, discount: 10.0, total_price: 60.0},
            {product_code: 'B', quantity: 1, unit_price: 30.0, discount: 0, total_price: 30.0}
          ],
          cart_discount: 0.0,
